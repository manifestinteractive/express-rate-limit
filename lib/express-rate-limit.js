...
function RateLimit(options) {
  ...
  function rateLimit(req, res, next) {
    ...
    var headerMax = (options.max === 0) ? 'Unlimited' : options.max;
    var headerRemaining = (options.max === 0) ? 'Unlimited' : rateRemaining;
    
    res.setHeader('X-Rate-Limit-Limit', headerMax);
    res.setHeader('X-Rate-Limit-Remaining', headerRemaining);
